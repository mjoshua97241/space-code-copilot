...
### Step 5: Blueprint Extraction (45 seconds)

**Talking Points:**
- "The system can extract structured room data directly from blueprint images using Vision LLM"
- "This eliminates the need to manually create CSV schedules"
- "VLM understands room labels, classifies types, and associates dimensions with rooms"

**Action:**
1. Switch to "🔍 Blueprint Extraction" tab
2. Show the upload interface (drag-and-drop or file picker)
3. Upload a sample blueprint PDF or image
4. Click "Extract Rooms" button
5. Show the extraction results table:
   - Room names, areas (editable), confidence scores
   - Explain that areas are editable if needed
6. Click "Check Compliance & Generate Overlays" button
7. Show compliance results:
   - Compliance column appears with tooltips
   - Non-compliant rooms highlighted
8. Explain the workflow:
   - Upload blueprint → Extract rooms → Edit areas (if needed) → Check compliance
   - Extracted rooms can be used in chat queries (blueprint context)

**Expected Behavior:**
- Upload shows blueprint image in plan viewer
- Extraction shows table with extracted rooms (name, area, confidence)
- Areas are editable input fields
- Compliance check shows violations with tooltips
- Extracted rooms are available for blueprint context in chat

**Key Features to Highlight:**
- VLM semantic understanding (not just OCR)
- Room type classification (bedroom, bathroom, kitchen, etc.)
- Dimension association (VLM associates area measurements with room labels)
- Editable areas for user correction
- Direct compliance checking on extracted data
- Integration with conversational chat (blueprint context)

**Note:** VLM label overlays are generated by the backend but frontend rendering is deferred to future. The system extracts rooms and generates overlay bounding boxes, but visual overlay rendering in the plan viewer is not yet implemented.

---

### Step 6: PDF Upload for Building Codes (30 seconds)
...
- "Key features: Automated compliance checking, VLM-based blueprint extraction, conversational chat with blueprint context, PDF upload for custom codes"
...
- Highlight the key features:
  ...
  3. VLM-based blueprint extraction (semantic understanding of room labels and dimensions)
  4. Conversational RAG-powered code Q&A with blueprint context
  5. PDF upload for custom building codes
...
- **"Are overlays displayed for extracted blueprints?"** → VLM label overlays are generated by the backend but frontend rendering is deferred to future. The system extracts rooms and generates overlay bounding boxes, but visual overlay rendering in the plan viewer is not yet implemented
- **"What VLM models are evaluated?"** → Currently evaluated GPT-4o and Gemini 2.0 Flash (Gemini 2.0 Flash selected as default). Hugging Face VLM evaluation is deferred (requires CUDA GPU)
