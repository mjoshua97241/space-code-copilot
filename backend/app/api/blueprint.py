...
async def extract_and_check_compliance(
    ...
    scale: Optional[float] = Form(None, description="Scale factor (default 1.0 for 1:100)"),
    page_index: Optional[int] = Form(None, description="PDF page index (0-based). If None, extracts all pages combined."),
    generate_overlays: bool = Form(True, description="Whether to generate overlays from OCR"),
    ...
    2. OCR-based overlay generation (optional)
    ...
    Args:
        ...
        generate_overlays: Whether to generate overlays from OCR
        use_opencv: Whether to use OpenCV for advanced boundary detection (requires opencv-python)
        
    Returns:
        Dictionary with:
        - "extraction": BlueprintExtractionResult (with overlays if generate_overlays=True)
    ...
    try:
        # Step 1: Extract rooms from blueprint
        result = extract_rooms_from_blueprint(
            image_path=tmp_path,
            scale_override=scale,
            page_index=page_index
        ...
        # Step 2: Generate overlays (if requested)
