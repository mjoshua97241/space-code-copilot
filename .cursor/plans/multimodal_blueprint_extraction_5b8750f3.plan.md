...
2. **Semantic extraction focus**: VLM reads labels, classifies types, associates dimensions
...
**Changes**: Added vision-capable LLM support for GPT-4o and Gemini 1.5 Flash Vision.
...
**Prompt strategy**: Emphasizes semantic understanding - reading labels, classifying types, associating dimensions, applying scale.
...
**File**: `backend/app/api/blueprint.py`
...
**Implementation**: Handles file upload, calls `extract_rooms_from_blueprint()`, returns `BlueprintExtractionResult` directly.
...
**Changes**: Added blueprint upload section with:
...
- Numeric ranges: area_m2 > 0, reasonable bounds (2.0 to 500.0 m²) ✅
- Type classification: validate against known room types (VALID_ROOM_TYPES set) ✅
- Confidence scoring: based on extraction quality heuristics ✅

**Enhanced validation includes**:

- Room type validation against `VALID_ROOM_TYPES` set
- Area bounds checking (MIN_ROOM_AREA_M2 = 2.0, MAX_ROOM_AREA_M2 = 500.0)
- Name quality checks (non-empty, trimmed)
- Level validation (>= 1)

**Enhanced confidence scoring includes**:

- Name confidence: Detects generic names ("Room 1") vs specific names
- Type confidence: Penalizes "other" types (LLM couldn't classify)
- Area confidence: Checks reasonableness and consistency
- Overall: Weighted combination (type 40%, area 30%, name 30%)
...
- Type classification: 100% match rate ✅
- Recall: 45.56% average (needs improvement)
- Precision: 55.79% average (needs improvement)
- Area accuracy: 65.38% average (good for matched rooms, affected by room splitting)
- Major issues: Room splitting, missing small rooms, multi-level extraction problems
...
- Extract rooms with >85% area accuracy on curated plans (Current: 65.38% - needs improvement)
- Correct room type classification >90% (Current: 100% ✅)
- Recall >80%, Precision >85% (Current: 45.56% recall, 55.79% precision - needs improvement)
- Semantic understanding score >0.7 (To be measured in Phase 6)
- Composite score >0.75 (To be measured in Phase 6)
...
- Future work: auto-scale detection, door extraction, more plan types# Multimodal Blueprint Extraction Implementation Plan
...
def calculate_area_accuracy(extracted_rooms: List[Room], ground_truth_rooms: List[Room]) -> float:
    """Calculate area accuracy using MAPE"""
    # Match rooms, calculate percentage errors, return 1.0 - MAPEdef calculate_recall(extracted_rooms: List[Room], ground_truth_rooms: List[Room]) -> float:
    """Calculate recall: % of ground truth rooms found"""
...
- **Phase 2**: 0.5 day (API endpoint) ✅
- **Phase 3**: 1 day (frontend integration) ✅
- **Phase 4**: 1 day (validation & testing) 🔄 (validation done, testing pending)
- **Phase 5**: 0.5 day (dependencies & config)
- **Phase 6**: 1-1.5 days (VLM metrics framework)
